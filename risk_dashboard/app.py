...
def page_risk_overview():
    ...
    # ── Risk Timeline ─────────────────────────────────────────
    st.markdown(sec("trend", "Risk Trend Timeline"), unsafe_allow_html=True)
    st.caption(
        "Weekly model risk per resource. Shaded bands mark documented tightening "
        "episodes; dotted lines are the Watch / Elevated / Critical thresholds."
    )

    fig = go.Figure()
    if timeline_df.empty:
        st.warning("Risk timeline has not been generated yet — run the export adapter.")
    else:
    ...
    # Threshold lines — labelled on the LEFT so they never collide with the legend.
    _thr_label = {"low": "Watch", "medium": "Elevated", "high": "Critical"}
    for name, val in RISK_THRESHOLDS.items():
        fig.add_hline(
            y=val, line_dash="dot", line_color="#cbd5e1", line_width=1,
            annotation_text=_thr_label.get(name, name.capitalize()),
            annotation_position="top left",
            annotation_font=dict(color="#1f2937", size=10),
        )
...
def page_model_performance():
    ...
    # ── ROC and PR Curves ─────────────────────────────────────
    _roc_img = OUTPUTS_DIR / "roc_curves.png"
    _pr_img = OUTPUTS_DIR / "pr_curves.png"
    _cm_img = OUTPUTS_DIR / "confusion_matrices.png"

    col1, col2 = st.columns(2)
    with col1:
        st.markdown(sec("roc", "ROC Curves"), unsafe_allow_html=True)
        if _roc_img.exists():
            st.image(str(_roc_img), use_container_width=True,
                     caption="ROC curves per model — true-positive vs false-positive rate; "
                             "higher and further left is better (AUC in legend).")
        else:
            st.warning("ROC curves not generated yet — run the training pipeline.")
    with col2:
        st.markdown(sec("pr", "Precision–Recall Curves"), unsafe_allow_html=True)
        if _pr_img.exists():
            st.image(str(_pr_img), use_container_width=True,
                     caption="Precision–recall per model against the positive-class baseline; "
                             "higher curves are better (AP in legend).")
        else:
            st.warning("Precision–recall curves not generated yet.")

    # ── Confusion Matrices ────────────────────────────────────
    st.markdown(sec("matrix", "Confusion Matrices"), unsafe_allow_html=True)
    if _cm_img.exists():
        st.image(str(_cm_img), use_container_width=True,
                 caption="Per-model confusion matrices on the held-out test weeks: rows are "
                         "actual (No Risk / Risk), columns are predicted.")
    else:
        st.warning("Confusion matrices not generated yet.")
...
def page_explainability():
    ...
    # ── Saved SHAP Plots ──────────────────────────────────────
    _shap_bar = OUTPUTS_DIR / "shap" / "bar_plot.png"
    _shap_summary = OUTPUTS_DIR / "shap" / "summary_plot.png"

    if _shap_bar.exists() or _shap_summary.exists():
        st.markdown(sec("drivers", "SHAP Plots (from training)"), unsafe_allow_html=True)
        shap_plot_cols = st.columns(2)
        if _shap_bar.exists():
            with shap_plot_cols[0]:
                st.image(str(_shap_bar), caption="SHAP feature-importance bar plot generated during training.", use_container_width=True)
        if _shap_summary.exists():
            with shap_plot_cols[1]:
                st.image(str(_shap_summary), caption="SHAP summary (beeswarm) plot from training.", use_container_width=True)

    # ── SHAP Summary Beeswarm ─────────────────────────────────
    if shap_values is not None:
        st.markdown(sec("drivers", "SHAP Summary (Beeswarm)"), unsafe_allow_html=True)

        # Create a beeswarm-like scatter plot
        fig_bee = go.Figure()
        for i, feat in enumerate(feature_names):
            ...
            fig_bee.add_trace(go.Scatter(
                x=vals,
                y=[feat.replace("_", " ").title()] * len(vals),
                mode="markers",
                marker=dict(
                    size=4,
                    color=vals,
                    colorscale=[[0, "#58a6ff"], [0.5, "#c9d1d9"], [1, "#f85149"]],
                    opacity=0.6,
                ),
        ...
        fig_bee.update_layout(
            **PLOTLY_LAYOUT,
            height=max(400, len(feature_names) * 45),
            title=dict(text="SHAP Value Distribution by Feature", font=dict(size=15)),
            xaxis_title="SHAP Value",
        )
        st.plotly_chart(fig_bee, width="stretch", theme=None)
    ...
    for feat, val in list(zip(wf_feats, wf_vals))[:5]:
        ...
        driver_parts.append(
            f'<span style="color:{color}; font-weight:600;">{pretty_feature(feat)} '
            f'<span style="font-weight:500;">{direction}</span></span>')
...
def page_case_studies():
    ...
    _elevated = ("high", "very high", "strong", "sustained", "rising", "persistent",
                 "sold-out", "allocation", "backlog", "critical", "crisis", "long")
    for feat, val in case["key_features"].items():
        low = val.lower()
        color = "#b91c1c" if any(k in low for k in _elevated) else "#111827"
        feat_table += f'<tr><td>{feat}</td><td style="color:{color}; font-weight:600;">{val}</td></tr>'
...
def main():
    ...
    with st.sidebar:
        st.markdown(
            ...
            f'<div style="font-size:10px; color:#94a3b8; letter-spacing:2.5px; margin-top:3px;">'
        ...
        st.markdown(
            ...
            'v2 &bull; Supply Chain Risk<br>Intelligence Framework<br>'
            '<span style="color:#7c8aa3;">Datasets: Risk Overview → Downloads</span></div>',
            unsafe_allow_html=True,
        )
